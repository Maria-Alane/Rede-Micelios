...
@HiltViewModel
class HyphaDetailViewModel @Inject constructor(
    ...
    fun loadHypha(hyphaId: String) {
        if (hasLoaded) return
        hasLoaded = true

        viewModelScope.launch {
            hyphaRepository.getHyphaById(hyphaId).collect { _hypha.value = it }
        }

        viewModelScope.launch {
            hyphaRepository.getMembersByHypha(hyphaId).collect { _members.value = it }
        }

        viewModelScope.launch {
            momentRepository.getMomentsByHypha(hyphaId).collect { _moments.value = it }
        }
    }
}